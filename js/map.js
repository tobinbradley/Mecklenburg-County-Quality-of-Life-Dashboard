...
function addMarker(lon, lat, featuretype, label) {
    ...
    var markerIcon = { icon: 'img/marker.png' };
...
function performIntersection(lat, lon) {
    url = "https://www.googleapis.com/fusiontables/v1/query?sql=SELECT ID," + getFieldsArray(FTmeta).join() + " FROM " + tableID + " WHERE ST_INTERSECTS(geometry, CIRCLE(LATLNG(" + lat + "," + lon + "),1))&key=" + apiKey + "&callback=?";
...
function selectNeighborhoodByID(idvalue) {
    ...
    url = "https://www.googleapis.com/fusiontables/v1/query?sql=SELECT ID," + getFieldsArray(FTmeta).join() + " FROM " + tableID + " WHERE ID = " + idvalue + "&key=" + apiKey + "&callback=?";
    $.getJSON(url, function(data) {
        if (data.rows.length > 0) {
            assignData(data);
    ...
    if (measure.style.type == "range") {
        $.each(measure.style.breaks, function(index, value) {
            if (index == measure.style.breaks.length - 1) {
                mapStyleJSON.push({
                    where: measure.field + " > " + value,
                    polygonOptions: {
                        fillColor: measure.style.colors[index],
                        fillOpacity: theOpacity
                    }
                });
            } else if (index === 0) {
                mapStyleJSON.push({
                    where: measure.field + " >= 0 and " + measure.field + " <= " + measure.style.breaks[index + 1],
                    polygonOptions: {
                        fillColor: measure.style.colors[index],
                        fillOpacity: theOpacity
                    }
                ...
                mapStyleJSON.push({
                    where: measure.field + " > " + value + " and " + measure.field + " <= " + measure.style.breaks[index + 1],
                    polygonOptions: {
                        fillColor: measure.style.colors[index],
                        fillOpacity: theOpacity
                    }
...
function createLegend(measure) {
    ...
    // empty div
    $("#legend").empty();
    $("#legend").append("<strong>" + $("#mapIndicie option:selected").text() + "</strong>");
